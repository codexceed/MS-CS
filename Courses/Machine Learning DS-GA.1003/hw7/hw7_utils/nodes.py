...
class L2NormPenaltyNode(object):
    ...
    def backward(self):
        d_x = 2 * self.d_out * self.l2_reg * self.w.out
        self.w.d_out += d_x
        return self.d_out
...
class AffineNode(object):
    ...
    def backward(self):
        d_x = self.W.out.reshape(self.W.out.shape[-1], -1) @ self.d_out.reshape(-1, 1)
        d_w = self.d_out.reshape(-1, 1) @ self.x.out.reshape(1, -1)
        d_b = self.d_out.reshape(-1, 1)
        self.x.d_out = self.x.d_out.reshape(-1, 1) + d_x
        self.W.d_out += d_w.reshape(self.W.d_out.shape)
        self.b.d_out = self.b.d_out.reshape(-1, 1) + d_b
...
class TanhNode(object):
    ...
    def backward(self):
        d_a = self.d_out * (1 - self.out ** 2).reshape(self.out.shape[0], -1)
        self.a.d_out = self.a.d_out.reshape(d_a.shape) + d_a
